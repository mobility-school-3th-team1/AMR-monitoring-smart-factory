...
| 순서 | ID | 요약 | 주 영역 | 녹화 화면 |
| --- | --- | --- | --- | --- |
| 1 | [D-01](#d-01-대시보드-비상-상황-발생-버튼) | 대시보드 비상 발생 버튼 | FE (+ BE 기존 API) | SCR-01 | [x] |
| 2 | [D-02](#d-02-idle-amr-랜덤-작업-할당--20초-완료) | IDLE 랜덤 작업·20초 완료 | BE, DAS | SCR-01, SCR-05 | [x] |
| 3 | [D-03](#d-03-배터리-감소-idle--operating) | IDLE/OPERATING 배터리 감소 | BE | SCR-01~04 | [x] |
| 4 | [D-04](#d-04-충전-중-배터리-5초) | CHARGING 배터리 +5%/초 | BE | SCR-04 | [x] |
| 5 | [D-05](#d-05-amr-목록-상태-필터) | AMR 목록 상태 필터 | FE (BE 쿼리 기존) | SCR-02 | [x] |
| 6 | [D-06](#d-06-충전-스테이션1-충전-중-amr-목록) | 스테이션 1 충전 중 AMR 목록 | BE, FE | SCR-04 | [x] |
| 7 | [D-07](#d-07-완충-예상-시간-표시) | 완충 예상(산술) UI | FE | SCR-04 | [x] |
| 8 | [D-08](#d-08-충전-완료-예상-도넛-차트) | 충전 완료 예상 도넛 | FE | SCR-04 | [x] |
| 9 | [D-09](#d-09-작업-이력-동기화) | 작업 이력·분석 동기화 | BE, FE | SCR-05 | [x] |
...
- [x] D-02 `AmrDemoSimulationService`: 작업 할당·완료 스케줄
- [x] D-03·D-04 배터리 tick (`app.demo.*` 설정)
- [x] D-06 `ChargingService.listStations()` — 스테이션 1 `amrs` 실데이터
- [x] D-09 `WorkHistory` / `Analytics` — 시뮬레이션 `AMR_TASK` 반영(기존 API)

### FE (`FE/frontend`)

- [x] D-01 `DashboardView.vue` 비상 버튼
- [x] D-05 `AmrListView.vue` 상태 필터
- [x] D-06~D-08 `BatteryView.vue` 충전 목록·완충 예상·도넛
- [x] D-09 `WorkHistoryView.vue` 폴링 연동(추가 변경 없음)

### DAS (`docker/node-red`)

- [x] D-02: DB `OPERATING` ↔ MQTT 이동(기존 플로우 유지)
- [x] D-01: 비상 시 `mqttStopLatch` + DB 동기화(기존 플로우 유지)
...
| 순서 | ID | 요약 | 주 영역 |
| --- | --- | --- | --- |
| 1 | D-10 | 배터리 ≤20%: 작업 종료 후 `CHARGING`, 맵 (50%, 80%) | BE, DAS |
| 2 | D-11 | 배터리 ≤5%: `STOPPED` 정지 | BE, FE |
| 3 | D-12 | 대시보드 비상 버튼: **알림만**(자동 E-stop 금지) | FE |
| 4 | D-13 | AMR 개별 관제: 번호 입력으로 AMR 선택 | FE |
| 5 | D-14 | 충전 완료 예상 도넛 실데이터 반영 | FE |
| 6 | D-15 | AMR 목록: 사용자 지정 상태 정렬 순서 | FE |

### D-10: 저배터리 → 충전

**상태:** [x] 완료

- 작업 `IN_PROGRESS` 완료 시 배터리 ≤20% → `CHARGING`, 충전소 1 세션
- `IDLE`이고 ≤20%이면 새 작업 대신 충전 진입
- DAS: `CHARGING` AMR 좌표 **x=50, y=80** (%)

### D-11: 극저배터리 정지

**상태:** [x] 완료

- 배터리 ≤5% → `STOPPED` (이동·작업 할당 중단)

### D-12: 비상 **상황** 발생 (알림)

**상태:** [x] 완료

- SCR-01 버튼은 시나리오·알람만 발생
- SCR-03 **비상 정지**는 사용자가 직접 실행

### D-13: AMR 번호 선택

**상태:** [x] 완료

- SCR-03 숫자 입력(예: 1~5) → `amr-0N` 조회

### D-14: 도넛 차트 실데이터

**상태:** [x] 완료

- `GET /amrs?status=CHARGING` 등 실충전 데이터로 버킷·도넛 갱신

### D-15: 상태 정렬

**상태:** [x] 완료

- SCR-02 상태 코드 순서 입력(쉼표 구분), `localStorage` 유지
...
| 순서 | ID | 요약 | 영역 |
| --- | --- | --- | --- |
| 1 | D-16 | AMR 가로/세로 경로만 이동 (구역별 좌표 문서) | DAS |
| 2 | D-17 | 환경 센서 10초마다 ±0.1 변동 | DAS |
| 3 | D-18 | 충전 좌표 (5%, 15%), 이동 후 충전 | BE, DAS |
| 4 | D-19 | KPI 「에러 AMR의 수」(STOPPED+ERROR 등) | FE |
| 5 | D-20 | 비상 시연 버튼 → 대상 AMR 실제 ERROR | BE, FE |
| 6 | D-21 | 충전 시연 버튼 추가 (배터리 ≤20% 후 충전 플로우) | BE, FE |
| 7 | D-22 | 시연 버튼 소형·구석 배치 | FE |
| 8 | D-23 | 작업 이력 차트 실시간·1분 bin | BE, FE |
| 9 | D-24 | 충전 완료 예상 도넛 0건 슬라이스 버그 수정 | FE |
| 10 | D-25 | 완충 예상 레이블 5초/10초 구간 | FE |

### D-16 ~ D-25 상세

**상태:** [x] 완료

- D-16: `docs/공장 구역별 AMR 경로 좌표.md` 구역 범위 내 축 정렬 이동
- D-17: `build_environment` 주기 10s, 변동량 0.1
- D-18: `EN_ROUTE_CHARGING` → 좌표 도착 후 `CHARGING`·배터리 증가
- D-19~D-22: `DashboardView` KPI·시연 API·UI
- D-23: `GET /analytics/workload?groupBy=minute` + SCR-04 차트
- D-24~D-25: `demo-simulation.js`·`BatteryView` 도넛
...
| 순서 | ID | 요약 | 영역 |
| --- | --- | --- | --- |
| 1 | D-26 | 비상 시연 → 목록/개별 관제 ERROR 표시 | BE, FE |
| 2 | D-27 | 충전소 도착 후에만 배터리 충전 | BE |
| 3 | D-28 | 작업 이력 시간 로컬(Asia/Seoul) 정합 | BE, FE |
| 4 | D-29 | 완충 예상 도넛 0건 슬라이스 제거 | FE |
| 5 | D-30 | 완충 예상 중空 도넛(CSS 레이어) | FE |
...
| 순서 | ID | 요약 | 영역 |
| --- | --- | --- | --- |
| 1 | D-31 | 랜덤 작업 배정 시 출발지·도착지 상이 | BE |
| 2 | D-32 | AMR 개별 관제: 사이드바 제거, 목록 선택 시 모달 | FE |
...
| 2026-05-20 | 시연 수정 2차 D-10~D-15 추가 |
| 2026-05-20 | 시연 수정 3차 D-16~D-25 추가 |
| 2026-05-20 | 시연 수정 4차 D-26~D-30 추가 |
| 2026-05-20 | 시연 수정 5차 D-31~D-32 추가 |
