---
adr_id: "20260515-1112-BE-envsetup"
status: "Accepted"
...
- 선택 이유
  ...
  - H2는 스키마 실험·데모 시드에 비용이 낮고, 이후 `docker/mysql/init.sql` 기반 MySQL로 전환 경로가 분리된다.
...
| Flyway / Liquibase | `docker/mysql/init.sql` 정합 후 도입 검토 |
...
| C. A + H2/JPA 임시 연동 + Docker 단독 실행 | API·Repository·시드까지 병렬 검증 가능, 로컬 환경 표준화 | H2와 물리 스키마(`docker/mysql/init.sql`) 불일치·이중 작업 위험 | **채택** |
...
### 리스크 및 대응
- 리스크: H2 자동 DDL과 `docker/mysql/init.sql` 물리 스키마·테이블명이 어긋날 수 있다.
- 대응: MySQL 연동을 1순위 백로그로 두고, 엔티티·Repository를 `docker/mysql/init.sql` 기준으로 정합한다(`BE/TODO.md`).
...
## 미해결 이슈
- [ ] JPA 엔티티를 `docker/mysql/init.sql` 물리 스키마에 맞게 정합하고 MySQL 드라이버로 전환
